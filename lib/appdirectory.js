...
var userData = function(roaming, platform) {
    ...
    if (platform === "darwin") {
        ...
        dataPath = path.join(process.env[sysVariable], '{1}', '{0}')
    } else {
        if (process.env.XDG_DATA_HOME || process.env.XDG_DATA_HOME !== "") {
...
var siteData = function(platform) {
    ...
    if (platform === "darwin") {
        ...
        if (process.env.XDG_DATA_DIRS || process.env.XDG_DATA_HOME !== "") {
...
AppDirectory.prototype = {
    ...
    set useRoaming(bool) {
        ...
        this._userDataTemplate = userData(bool, this._platform)
    },
    get platform() {
    ...
    set platform(str) {
        this._platform = str
        this._userDataTemplate = userData(this._useRoaming, str)
    },
    userData: function() {
